...
parser.add_argument('--beita', default=0.1, type=float, help='for HE')
...
def compute_mat_MHN(stored_tensor):
    ...
    slse.summary()
    steps_per_epoch = 50
    #lr_schedule =  ExponentialWarmupSchedule(lr=0.02, decay_start=steps_per_epoch*100, decay_ratio=0.1)
...
def compute_mat_MHN_all(stored_tensor):
    h_dim = args.hidden
    data_feature = stored_tensor
    if args.kmeans == 0:
        print(stored_tensor.shape,stored_tensor.shape[0],stored_tensor.shape[1])
        if stored_tensor.shape[0] > h_dim:
            stored_feature = stored_tensor[:h_dim] #[B_stored,dim]
        else:
            h_dim = stored_tensor.shape[0]
            stored_feature = stored_tensor[:h_dim]
            print(stored_feature.shape,h_dim)
    ...
    slse.summary()
    steps_per_epoch = 50
    lr_schedule =  ExponentialWarmupSchedule(lr=0.0002, decay_start=steps_per_epoch*50, decay_ratio=0.1)
    #optimizer = tf.keras.optimizers.Adam(beta_1=0.9, beta_2=0.98, epsilon=1e-9,learning_rate = lr_schedule)
    optimizer = tf.keras.optimizers.Adam(beta_1=0.9, beta_2=0.98, epsilon=1e-9)
    slse.encoder.set_weights([stored_feature.T])
    #print(slse.trainable_variables)
...
if args.read_mode == 0:
    ...
    torch.save(MHN_tensor,'./stored_pattern/MHN_all_mat/size_{}/{}/{}/stored_all_epoch_{}_hidden_{}.pth'.format(args.resize_val,args.dataset,args.model,args.epoch,args.hidden))
else:
    path = './stored_pattern/MHN_all_mat/size_{}/{}/{}/stored_all_epoch_{}_hidden_{}.pth'.format(args.resize_val,args.dataset,args.model,args.epoch,args.hidden)
...
bp3 = ax3.boxplot([ID_score,OOD_score])
plt.show()
...
bp4 = ax4.boxplot([ID_score,OOD_score])
plt.show()
...
bp5 = ax5.boxplot([ID_score,OOD_score])
plt.show()
...
bp0 = ax0.boxplot([ID_score,OOD_score])
plt.show()
...
bp6 = ax6.boxplot([ID_score,OOD_score])
plt.show()
...
bp7 = ax7.boxplot([ID_score,OOD_score])
plt.show()
...
bp8 = ax8.boxplot([ID_score,OOD_score])
plt.show()
