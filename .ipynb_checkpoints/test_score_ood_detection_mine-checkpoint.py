    ...
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(loader):
            ...
            if args.score == 'SHE':
                ...
                output,penultimate = net(data,need_penultimate=args.need_penultimate)
                _score.extend(compute_score_MHN(prediction=output,penultimate=penultimate))
            elif args.score == 'MSP':
                output,penultimate = net(data,need_penultimate=args.need_penultimate)
...
def simple_compute_score_HE(prediction,penultimate,need_mask=False):
    ...
    for i in range(numclass):
        ...
        if total_stored_feature is None:
            total_stored_feature = stored_tensor/torch.norm(stored_tensor)
            meen_stored_feeture = stored_tensor
        else:
            total_stored_feature = torch.cat((total_stored_feature,stored_tensor/torch.norm(stored_tensor)),dim=0)
            meen_stored_feeture = stored_tensor + meen_stored_feeture
    #--------------------------------------------------------------------------------------

    #target = total_stored_feature[pred,:]
    v = torch.mm(total_stored_feature,torch.t(total_stored_feature)).cpu().numpy()
    #print(v)
    target = meen_stored_feeture
    res = []
...
def compute_score_MHN(prediction,penultimate,need_mask=False):
    ...
    for i in range(numclass):
        ...
        for _ in range(5):
            res_energy_score = torch.mm(stored_feature,test_feature) #[B_stored,B_test]
            res_exp = torch.exp(res_energy_score*args.beita)
            res_mySoftMax = res_exp/torch.max(torch.sum(res_exp,dim=0),alp)
            test_feature = torch.mm(stored_feature.transpose(0, 1),res_mySoftMax)
        lse_res = -to_np(torch.sum(res_exp)) #[1,B_test]
        res.append(lse_res)
    return res  
