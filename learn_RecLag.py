...
class Sampler(Model):
    ...
    def call(self,z):
        ...
        #sample = tf.transpose(tf.random.categorical(tf.math.log(p), num_samples=self.n_sample))
...
def compute_mat_MHN(stored_tensor):
    ...
    for epoch in range(EPOCHS):
        ...
        if (epoch+1)%10==0:
            print(f'Epoch {epoch+1}, Batch {batch}, Train Loss {np.array(train_loss).mean()}')
